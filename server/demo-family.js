// The fictional demo family — the crowd all map work is judged on, and the
// only data that ever appears in screenshots of this public repository.
// Every person here is invented.
//
// It lives in server/ rather than tools/ so it ships inside the container:
// a test instance can be filled from /admin with one click, no shell.
// `tools/seed-demo.mjs` is the command-line wrapper around the same code.
import {
  createPerson, createUnion, addChild, upsertRelationship, createBranch, createStory,
} from './routes.js';
import { db, tx } from './db.js';

/** People who are not just somebody's account — what "empty enough" means. */
export const strangersInTree = () => db.prepare(`SELECT COUNT(*) c FROM persons
  WHERE id NOT IN (SELECT person_id FROM users WHERE person_id IS NOT NULL)`).get().c;

/**
 * Fill an empty tree with four generations of two families joined by a
 * marriage. Refuses as soon as there is anyone real in here: this is for a
 * fresh test instance, never for the family's own tree.
 */
export function seedDemoFamily() {
  if (strangersInTree() > 0) {
    throw Object.assign(new Error('err.demo_not_empty'), { status: 400 });
  }
  ...
  return tx(() => {
    ...
    // ------------------------------------------------------------ coordinates
    // Fixed, so the seed never calls Nominatim — the Orte map has pins out of
    // the box, and the demo works offline.
    const PLACES = {
    ...
    return {
      ...
      unions: db.prepare('SELECT COUNT(*) c FROM unions').get().c,
      stories: db.prepare('SELECT COUNT(*) c FROM stories').get().c,
      // Whoever the tree reads best from — handed back so the app can offer
      // to look through their eyes instead of the empty account person.
      proband: P.thomas,
    };
  });
}
